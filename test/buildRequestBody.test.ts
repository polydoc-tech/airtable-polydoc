...
function base(overrides: Partial<PolyDocParams>): PolyDocParams {
  return {
    ...
    delivery: { mode: 'download', binaryPropertyName: 'data' },
    ...overrides,
  };
}
...
describe('buildRequestBody', () => {
  ...
  it('drops a non-positive timeout', () => {
    const r = buildRequestBody(base({ timeout: 0 }));
    expect(r.body).not.toHaveProperty('timeout');
  });
});
...
describe('defaultFilename', () => {
  it('uses document.pdf for PDF and e-invoice', () => {
    expect(defaultFilename('pdf')).toBe('document.pdf');
    expect(defaultFilename('einvoice')).toBe('document.pdf');
  });

  it('maps screenshot image type to an extension, jpeg becomes jpg', () => {
    expect(defaultFilename('screenshot', 'png')).toBe('screenshot.png');
    expect(defaultFilename('screenshot', 'jpeg')).toBe('screenshot.jpg');
    expect(defaultFilename('screenshot')).toBe('screenshot.png');
  });
});

describe('extractApiErrorMessage', () => {
  it('reads message then error from a JSON object', () => {
    expect(extractApiErrorMessage({ message: 'bad', error: 'x' })).toBe('bad');
    expect(extractApiErrorMessage({ error: 'only-error' })).toBe('only-error');
  });

  it('parses a JSON string body', () => {
    expect(extractApiErrorMessage('{"message":"parsed"}')).toBe('parsed');
  });
  ...
  it('decodes an ArrayBuffer error body', () => {
    const buf = new TextEncoder().encode('{"error":"from-bytes"}').buffer;
    expect(extractApiErrorMessage(buf)).toBe('from-bytes');
  });
});
