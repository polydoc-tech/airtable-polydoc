/**
 * Pure PolyDoc request-body builder, shared by the Airtable Blocks SDK extension
 * and the Run-a-script recipes. Framework-agnostic and side-effect free so it can
...
function buildScreenshot(opts: JsonObject): JsonObject | undefined {
  ...
  if (typeof opts.viewportWidth === 'number' && typeof opts.viewportHeight === 'number') {
    const viewport: JsonObject = { width: opts.viewportWidth, height: opts.viewportHeight };
...
/** Assemble the PolyDoc request body and target endpoint from resolved params. */
export function buildRequestBody(params: PolyDocParams): PolyDocRequest {
  ...
  const body: JsonObject = { source: resolveSource(params) };
...
/** Best-effort extraction of PolyDoc's `{ error, message }` from an error response body. */
